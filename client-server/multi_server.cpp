...
{  
    int opt = TRUE;  
    int master_socket = 0;
    int addrlen , new_socket = 0;
    int client_socket[30]; // Intialized below
    int max_clients = 30;
    int activity, i , valread , sd = 0;  
    int max_sd = 0;  
    ...
    if( 0 > setsockopt(master_socket, SOL_SOCKET, SO_REUSEADDR | SO_REUSEPORT, (char *)&opt, sizeof(opt)))  
    {  
        perror("setsockopt");  
        exit(EXIT_FAILURE);  
    }  
